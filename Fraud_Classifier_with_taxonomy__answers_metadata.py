...
class ClaimMetadata(BaseModel):
    """Metadata extracted from claim documents"""
    customer_name: Optional[str] = Field(default=None, description="Name of the customer filing the claim")
    claim_date: Optional[str] = Field(default=None, description="Date when the claim was filed")
    claim_id: Optional[str] = Field(default=None, description="Unique claim identifier")
    customer_id: Optional[str] = Field(default=None, description="Customer identifier or account number")
...
class FraudAnalysisState(BaseModel):
    ...
    claim_metadata: Optional[ClaimMetadata] = Field(default=None, description="Basic claim metadata extracted from document")
...
def metadata_extraction(state: FraudAnalysisState) -> FraudAnalysisState:
    """Extract basic claim metadata from the document"""
    if not state.raw_text:
        print("  → WARNING: No document content available for metadata extraction")
        state.claim_metadata = ClaimMetadata()
        return state
    
    # Build metadata extraction prompt
    metadata_prompt = f"""
You are a metadata extraction specialist for fraud claims processing. 
Extract the following specific metadata fields from the claim document provided.

Document Content:
{state.raw_text}

Extract the following fields if they are present in the document:
1. customer_name: The full name of the person filing the claim
2. claim_date: The date when this claim was filed (format as YYYY-MM-DD if possible)
3. claim_id: Any unique claim number, reference number, or claim identifier
4. customer_id: Customer account number, policy holder ID, or customer identifier
...
- For IDs, extract the exact identifier as written
...
Return the extracted metadata in the specified JSON format.
"""
    
    try:
        # Use structured output for metadata extraction
        structured_llm = llm.with_structured_output(ClaimMetadata)
        result = structured_llm.invoke(metadata_prompt)
        state.claim_metadata = result
        print(f"  → Metadata extracted: {result.model_dump()}")
    except Exception as e:
...
def generate_suggested_answer(state: FraudAnalysisState) -> FraudAnalysisState:
    ...
    # Add extracted metadata context
    if state.claim_metadata:
        fraud_context += "\nClaim Information:"
        if state.claim_metadata.customer_name:
            fraud_context += f"\n- Customer: {state.claim_metadata.customer_name}"
        if state.claim_metadata.claim_date:
            fraud_context += f"\n- Claim Date: {state.claim_metadata.claim_date}"
        if state.claim_metadata.claim_id:
            fraud_context += f"\n- Claim ID: {state.claim_metadata.claim_id}"
    ...
    - Consider the Fraud Context provided
    - Include the claim metadata in the response (customer name, claim date, claim id)
...
def generate_report(state: FraudAnalysisState) -> Dict[str, Any]:
    ...
    # Flatten claim_metadata for easier CSV export
    if state.claim_metadata:
        metadata_dict = state.claim_metadata.model_dump()
...
fraud_graph.add_node("extract_metadata", metadata_extraction)
...
fraud_graph.add_edge("ingest", "extract_metadata")
fraud_graph.add_edge("extract_metadata", "analyze")
